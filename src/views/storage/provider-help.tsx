    ...
    <TooltipProvider delayDuration={150}>
      <Tooltip>
        <TooltipTrigger asChild onFocus={(event) => event.preventDefault()}>
          <button
            type="button"
            aria-label={`${PROVIDERS[provider]} 接入帮助`}
        ...
        <TooltipContent
          ...
          align="start"
          sideOffset={8}
          className="block w-96 max-w-[calc(100vw-2rem)] space-y-3 p-4 text-xs leading-relaxed"
        >
          <p className="font-semibold">{PROVIDERS[provider]} 接入步骤</p>
          <p>{guide.steps}</p>
          <dl className="space-y-2">
            {[
              ["名称", "自定义接入名称，便于区分账号，无需从云平台获取。"],
              ...guide.fields,
            ].map(([label, description]) => (
              <div key={label}>
                <dt className="font-semibold">{label}</dt>
                <dd className="opacity-80">{description}</dd>
              </div>
            ))}
          </dl>
